...
void Teacher::printSubInterface()
{
	cout << endl;
	cout << "欢迎" << username_ << "老师登录!" << endl;
	cout << endl;
...
void Teacher::showMyReservation()
{
	RecordManager rm;
	...
	for (int i = 0; i < rm.getSize(); i++) {
		Record* r = rm.vec_[i];
		if (r->userType_ == "2" && atoi(r->userId_.c_str()) == id_) {
			hasResult = true;
			r->printRecord();
		}
	}
...
void Teacher::cancelReservation()
{
	RecordManager rm;
	if (rm.getSize() == 0) {
		cout << "无预约记录" << endl;
	...
	cout << "请输入要取消的记录, 输入0返回上一级" << endl << endl;
	int index = 1;
	vector<int> v;  // 

	for (int i = 0; i < rm.getSize(); i++)
	{
		if (rm.vec_[i]->userType_ == "2" && id_ == atoi(rm.vec_[i]->userId_.c_str())) {
...
{
	cout << "职工号: " << id_ << "\t\t 姓名: " << username_ << "\t\t 密码: " << password_ << endl;
}
